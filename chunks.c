...
void createChunk(Chunk *chunk, GLfloat xAdd, GLfloat zAdd, int isFirstCreation, int flag, uint64_t key)
{
    ...
    memset(chunk->lightData, 0, sizeof(uint8_t) * ChunkWidthX * ChunkLengthZ * ChunkHeightY);
...
void enqueue(Queue *queue, Block *blockEntry) {
    if (queue->size == queue->capacity) {
        return; 
    }

    // appends to the end of the queue
    queue->items[queue->rear] = blockEntry;
    queue->rear = (queue->rear + 1) % queue->capacity;
    queue->size++;
}

Block *dequeue(Queue *queue) {
    if (queue->size == 0) {
        return NULL; 
    }

    // pops from the start of queue and returns it
    Block *result = queue->items[queue->front];
